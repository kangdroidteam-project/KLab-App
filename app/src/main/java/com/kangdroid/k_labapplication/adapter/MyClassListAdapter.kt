...
class MyClassListAdapter(val items : MutableLiveData<List<SimplifiedMyPageCommunity>>) : RecyclerView.Adapter<MyClassListAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(val binding : MyClassListRowBinding) : RecyclerView.ViewHolder(binding.root) {
        init {
            binding.root.setOnClickListener {
                itemClickListener?.onItemClick(this,it,items.value!![adapterPosition],adapterPosition)
            }
        }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.binding.apply {
            ...
            myClassName.text = items.value!![position].contentTitle
            myClassTime.text = items.value!![position].startTime.toString()
            myClassSupply.text =items.value!![position].contentNeeds

            if(!items.value!![position].isRequestConfirmed){
